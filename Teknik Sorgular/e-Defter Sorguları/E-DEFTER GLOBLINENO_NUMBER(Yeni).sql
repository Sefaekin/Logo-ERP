...
------------------------------------
SET @GLOBLINENO=0
SET @MONTH=12     --- Bu alana ay bilgisi yazýlýr
SET @YEAR=2018   --- Bu alana yýl bilgisi yazýlýr
------------------------------------
...
SELECT 
                EM.LOGICALREF
FROM 
                LG_028_01_EMFICHE EF WITH(NOLOCK)
                LEFT OUTER JOIN LG_028_01_EMFLINE EM WITH(NOLOCK) ON EM.ACCFICHEREF=EF.LOGICALREF 
WHERE 
...
IF @GLOBLINENO=0
BEGIN
SELECT @GLOBLINENO=ISNULL(MAX(GLOBLINENO),0) FROM LG_028_01_EMFLINE WHERE MONTH(DATE_)=@MONTH-1
END
UPDATE LG_028_01_EMFLINE SET GLOBLINENO=@GLOBLINENO+1 WHERE LOGICALREF = @LOGICALREF
